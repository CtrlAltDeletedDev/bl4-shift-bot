...
class ShiftCodeDatabase:
    ...
    async def add_or_update_code(self, code: str, reward: str, expires: Optional[str], 
        ...
        async with self.connection.cursor() as cursor:
            ...
            await cursor.execute("SELECT id, times_scraped FROM codes WHERE code = ?", (code,))
    ...
    async def get_statistics(self) -> Dict:
        ...
        async with self.connection.cursor() as cursor:
            ...
            await cursor.execute("""
                ...
                LIMIT 5
            """)
            stats['most_scraped'] = [(row[0], row[1]) for row in await cursor.fetchall()]
            
            return stats
    
    async def log_command_usage(self, command_name: str, user_id: str, guild_id: Optional[str] = None):
    ...
    async def add_notification_subscription(self, channel_id: str, guild_id: str) -> bool:
...
async def test_database():
    ...
    async with Database("test_shift_codes.db") as db:
        ...
        for code in codes:
            print(f"  - {code['code']}: {code['reward']} (scraped {code['times_scraped']} times)")
